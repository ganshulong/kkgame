...
cc.Class({
    ...
    showCard(list,len,showBg=false){
        for(let i=0;i<list.length;++i){
            let node = this.node.getComponent("ShiHuKa_Card").createCard(list[i],this._chairId==0?1:2);
            node.name = "card";
            if(this._chairId === 0) {
                ...
                if(showBg){
                    ...
                    this.node.getComponent("ShiHuKa_Card").createCard(0,1,true,child);
                    child.parent = node;
                    child.name = "bg";
                }
            }
    ...
    recvChiCard(data){
        ...
        // 我当前吃牌
        if((data.actionInfo.curaction.seat === cc.vv.gameData.getMySeatIndex() && 0 === this._chairId)||
            (cc.vv.gameData._isPlayBack && data.actionInfo.curaction.seat === this._seatIndex)){
            // 删除手里面的牌
