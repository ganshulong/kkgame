    ...
    initHandCard(list,parent,menzi,card=null,source=null){
        ...
        for(let i=0;i<tempList.length;++i){
            for(let j=0;j<tempList[i].length;++j) {
                let node = this.node.getComponent("PengHu_Card").createCard(tempList[i][j],1);
                node.scale = 0.5;
        ...
        for(let i=0;i<menzi.length;++i){
            ...
            for(let j=0;j<menzilist.length;++j){
                let node = this.node.getComponent("PengHu_Card").createCard(menzilist[j],0);
                node.scale = 0.45;
        ...
        if(card){
            ...
            this.node.getComponent("PengHu_Card").changCardBg(bgNode,source===0);
            let cardNode = this.node.getComponent("PengHu_Card").createCard(card,0);
            bgNode.y = 43;
            bgNode.x = width+60;
            bgNode.scale = 0.6;
