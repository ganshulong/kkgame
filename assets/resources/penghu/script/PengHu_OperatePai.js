...
cc.Class({
    ...
    addCard(card,showBg){
        ...
        for(let i=0;i<this._operateCardNode.childrenCount;++i){
            if(this._operateCardNode.children[i].cardValue == card ){
                ...
                if(this._operateCardNode.children[i].index !==3){
                    this.node.getComponent("PengHu_Card").createCard(card,2,showBg,this._operateCardNode.children[i]);
        ...
        if(num === 0){
            for(let i=0;i<4;++i){
                let node = this.node.getComponent("PengHu_Card").createCard(card,2,(i<3)?showBg:false);
            ...
            for(let i=0;i<4-num;++i){
                let node = this.node.getComponent("PengHu_Card").createCard(card,2,(num<3&&i==0)?showBg:false);
                node.y = node.height*(num+i);
                node.x = startX;
                node.cardValue = card;
                node.parent = this._operateCardNode;
