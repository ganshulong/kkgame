...
cc.Class({
    ...
    showCard(list,type,source=0,showAction = false){
        for(let i=0;i<list.length;++i){
            ...
            let node = this.node.getComponent("ShiHuKa_Card").createCard(list[i],showAction?1:2,showBg);
    ...
    showCardAction(node,startPos,endPos){
        ...
        node.runAction(cc.sequence(cc.spawn(cc.moveTo(time,endPos).easing(cc.easeOut(3)),cc.scaleTo(time,0.48),cc.fadeTo(time,50)),cc.callFunc(()=>{
            self.node.getComponent("ShiHuKa_Card").createCard(node.cardValue,2,node.showBg,node);
            node.scale = 1;
            node.opacity = 255;
        })))
    },
    ...
    addCard(card,showBg){
        ...
        for(let i=0;i<this._operateCardNode.childrenCount;++i){
            if(this._operateCardNode.children[i].cardValue == card ){
                ...
                if(this._operateCardNode.children[i].index !==3){
                    this.node.getComponent("ShiHuKa_Card").createCard(card,2,showBg,this._operateCardNode.children[i]);
        ...
        if(num === 0){
            for(let i=0;i<4;++i){
                let node = this.node.getComponent("ShiHuKa_Card").createCard(card,2,(i<3)?showBg:false);
            ...
            for(let i=0;i<4-num;++i){
                let node = this.node.getComponent("ShiHuKa_Card").createCard(card,2,(num<3&&i==0)?showBg:false);
                node.y = node.height*(num+i);
                node.x = startX;
                node.cardValue = card;
                node.parent = this._operateCardNode;
    ...
    recvPengNotify(data){
        ...
        if(data.actionInfo.curaction.seat === this._seatIndex){
            let card = data.actionInfo.curaction.card;
            this.showCard([card,card,card],data.actionInfo.curaction.type,0,true);      //扫碰 都显示三张
        }
    },
