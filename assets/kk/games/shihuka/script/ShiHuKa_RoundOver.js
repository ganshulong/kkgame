...
cc.Class({
    ...
    recvRoundOver(data){
        data = data.detail;
        if(this._layer === null){
            cc.loader.loadRes("common/prefab/Shihuka_round_over_view",(err,prefab)=>{
                if(err === null){
    ...
    showRoundInfo(data){
        ...
        if (panel_CardInfo.active) {
            ...
            if (data.hcard) {
                ...
                let cardNode = this.node.getComponent("ShiHuKa_Card").createCard(data.hcard,0);
            ...
            if (winerInfo) {
                ...
                for (let i = 0; i < winerInfo.menzi.length; i++) {
                    ...
                    for(let j = 0; j < cardArr.length; ++j){
                        let node = this.node.getComponent("ShiHuKa_Card").createCard(cardArr[j],2);
                ...
                for(let i=0; i < data.huCards.length; ++i){
                    ...
                    for(let j = 0; j < data.huCards[i].length; ++j) {
                        let node = this.node.getComponent("ShiHuKa_Card").createCard(data.huCards[i][j],2);
                        node.y = node.height * j;
                        node.parent = cardArrItem;
    ...
    initHandCard(list,parent,menzi,card=null,source=null){
        ...
        for(let i=0;i<tempList.length;++i){
            for(let j=0;j<tempList[i].length;++j) {
                let node = this.node.getComponent("ShiHuKa_Card").createCard(tempList[i][j],1);
                node.scale = 0.5;
        ...
        for(let i=0;i<menzi.length;++i){
            ...
            for(let j=0;j<menzilist.length;++j){
                let node = this.node.getComponent("ShiHuKa_Card").createCard(menzilist[j],0);
                node.scale = 0.45;
        ...
        if(card){
            let bgNode = new cc.Node();
            bgNode.addComponent(cc.Sprite);
            this.node.getComponent("ShiHuKa_Card").changCardBg(bgNode,source===0);
            let cardNode = this.node.getComponent("ShiHuKa_Card").createCard(card,0);
            bgNode.y = 43;
            bgNode.x = width+60;
            bgNode.scale = 0.6;
    ...
    onDestroy(){
        if(this._layer){
            cc.loader.releaseRes("common/prefab/Shihuka_round_over_view",cc.Prefab);
        }
    }
    // update (dt) {},
});
