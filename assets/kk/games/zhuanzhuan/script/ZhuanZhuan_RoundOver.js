...
cc.Class({
    ...
    recvRoundOverNotify(data){
        data = data.detail;
        if(this._layer === null){
            cc.loader.loadRes("common/prefab/ZhuanZhuan_round_over_view",(err,prefab)=>{
                if(err === null){
    ...
    showRoundInfo(data){
        ...
        if (panle_playerInfo.active) {
            for(let i = 0; i < data.users.length; ++i){
                ...
                if (data.seat === data.users[i].seat) {
                    let node = this.node.getComponent("ZhuanZhuan_Card").createCard(data.hcard);
                ...
                for(let j = 0; j < data.users[i].gangpai.length; ++j){
                    for (let k = 0; k < 4; k++) {
                        let node = this.node.getComponent("ZhuanZhuan_Card").createCard(data.users[i].gangpai[j]);
                ...
                for(let j = 0; j < data.users[i].pengpai.length; ++j){
                    for (let k = 0; k < 3; k++) {
                        let node = this.node.getComponent("ZhuanZhuan_Card").createCard(data.users[i].pengpai[j]);
                        node.parent = handCard;
                        node.x = curPosX;
                        curPosX += cardWidth;
                    }
                ...
                for(let j = 0; j < data.users[i].handInCards.length; ++j){
                    let node = this.node.getComponent("ZhuanZhuan_Card").createCard(data.users[i].handInCards[j]);
                    node.parent = handCard;
                    node.x = curPosX;
                    curPosX += cardWidth;
                }
        ...
        if (bg_zhongNiao.active) {
            ...
            for (let i = 0; i < data.bird.length; i++) {
                let node = this.node.getComponent("ZhuanZhuan_Card").createCard(data.bird[i]);
    ...
    onDestroy(){
        if(this._layer){
            cc.loader.releaseRes("common/prefab/ZhuanZhuan_round_over_view",cc.Prefab);
        }
    }
});
